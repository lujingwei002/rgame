...
struct LoginRequest {
    open_id: String, // 账号
    token: String, // token
    platform: config::PlatformType,//平台
}
...
async fn api_login(req: Request<hyper::body::Incoming>) -> Result<Response<BoxBody>, Box<dyn Error + Send + Sync>> {
    ...
    req.open_id = if let Some(n) = params.get("open_id") {
        ...
        return Ok(Response::builder().status(StatusCode::UNPROCESSABLE_ENTITY).body(full("missing open_id")).unwrap());
    };
    let platform = if let Some(p) = params.get("platform") {
        p.clone()
    } else {
        return Ok(Response::builder().status(StatusCode::UNPROCESSABLE_ENTITY).body(full("missing platform")).unwrap());
    };
